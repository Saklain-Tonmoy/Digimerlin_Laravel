<!--====== Header part start ======-->
<header id="header" class="header-two sticky-header">
    <!-- Header Menu  -->
    <div class="header-nav sticky-nav">
        <div class="container-fluid container-1600">
            <div class="nav-container">
                <!-- Site Logo -->
                <div class="site-logo">
                    <a href="#"><img src="{{asset('assets/frontend/images/digimerlin-logo.png')}}" alt="Logo"></a>
                </div>

                <!-- Main Menu -->
                <div class="nav-menu d-lg-flex align-items-center">

                    <!-- Navbar Close Icon -->
                    <div class="navbar-close">
                        <div class="cross-wrap"><span></span><span></span></div>
                    </div>

                    <!-- Menu Items -->
                    <div class="menu-items">
                        <ul>
                            <li><a href="#header">Home</a></li>
                            <li><a href="#services">Services</a></li>
                            <li><a href="#about">About</a></li>
                            <li><a href="#why-choose-us">Why Choose Us</a></li>
                            <li><a href="#client">Client</a></li>
                            <li><a href="#contact">Contact</a></li>
                        </ul>
                    </div>
                    <!-- Pushed Item -->
                    <div class="nav-pushed-item"></div>
                </div>

                <!-- Navbar Extra  -->
                <div class="navbar-extra d-flex align-items-center">
                    <!-- off canvas menu toggler-->
                    <div class="offcanvas-toggler">
                        <span><span></span><span></span><span></span></span>
                    </div>
                    <!-- Navbar Toggler -->
                    <div class="navbar-toggler">
                        <span></span><span></span><span></span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>
<!--====== Header part end ======-->

<!--====== OFF CANVAS START ======-->
<div class="offcanvas-wrapper">
    <div class="offcanvas-overly"></div>
    <div class="offcanvas-widget">
        <a href="#" class="offcanvas-close"><i class="fa fa-times"></i></a>
        <!-- About Widget -->
        <div class="widget about-widget">
            <div class="logo mb-30">
                <img src="{{asset('assets/frontend/images/digimerlin-logo.png')}}" alt="Image">
            </div>
            <p class="white-color">
                Digimerlin feeling pleased to present itself as one of the expert IT Solution companies in the online commercial world. We provide our customers the best IT Services they have ever experienced. We strongly consider our consumer loyalty.
            </p>
        </div>
        <!-- Contact information Widget -->
        <div class="widget contact-info-widget">
            <h5 class="widget-title white-color">Contact Us</h5>
            <ul>
                <li>
                    <i class="fa fa-map-marker"></i>
                    <span class="white-color">331/2 Tajuddin Ahmed Sarani, Moghbazar, Dhaka</span>
                </li>
                <li>
                    <i class="fa fa-phone"></i>
                    <a class="white-color" href="tel:(+880)1708-159827"> (+880)1708-159827</a>
                </li>
                <li>
                    <i class="fa fa-envelope-open"></i>
                    <a class="white-color" href="mailto:digimerlin2021@gmail.com">digimerlin2021@gmail.com</a>
                </li>
                <li>
                    <i class="fa fa-clock-o"></i>
                    <span class="white-color">Opening Hours: 09:30 am - 05:30 pm</span>
                </li>
            </ul>
        </div>
        <!-- Social Link -->
        <div class="widget social-link">
            <h5 class="widget-title white-color">Follow us</h5>
            <ul>
                <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                <li><a href="#"><i class="fa fa-google"></i></a></li>
            </ul>
        </div>
    </div>
</div>
<!--====== OFF CANVAS END ======-->